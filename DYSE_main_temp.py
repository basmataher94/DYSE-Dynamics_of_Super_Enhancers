...
# Identify stage-specific SEs
def stageSpecific(inputFile, ):
    stageFile = open(inputFile)
    if options.rose:
...
def main():
    ...
    # for fn in inputFiles:
        ...
        if options.rose:
            ...
            if options.stitch:
                # result = Parallel(n_jobs=12)(delayed(callRose)(f,rose_out,options.genome,options.stitch) for f in rosefiles)
                result = Parallel(n_jobs=12)(delayed(stageSpecific)(f) for f in inputFiles)
            else:
                # result = Parallel(n_jobs=12)(delayed(callRose)(f,rose_out,options.genome) for f in rosefiles)
                result = Parallel(n_jobs=12)(delayed(callRose)(f, rose_out, options.genome) for f in inputFiles)
            ...
            compileStageSE = groupSE(open(fn).read())
        stageFile.close()
