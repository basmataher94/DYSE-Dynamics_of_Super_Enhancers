...
#============================================================================
 ...
 calculate_cutoff <- function(inputVector, drawPlot=TRUE,...){
	...
	if(drawPlot){  #if TRUE, draw the plot
		...
		axis(1,sum(inputVector==0),sum(inputVector==0),col.axis="pink",col="pink",cex.axis=1.5) #Number of regions with zero signal
	}
...
cutoff_options <- calculate_cutoff(rankBy_vector, drawPlot=FALSE,xlab=paste(rankBy_factor,'_enhancers'),ylab=paste(rankByFactor,' Signal','- ',wceName),lwd=2,col=4,cex=1.5,cex.axis=1.5)
...
if(wceName == 'NONE'){
	plot(length(rankBy_vector):1,rankBy_vector[signalOrder], col='red',xlab=paste(rankBy_factor,'_enhancers'),ylab=paste(rankBy_factor,' Signal'),cex=2,pch=19)	
	
}else{
	plot(length(rankBy_vector):1,rankBy_vector[signalOrder], col='red',xlab=paste(rankBy_factor,'_enhancers'),ylab=paste(rankBy_factor,' Signal','- ',wceName),cex=2,pch=19)
}
abline(h=cutoff_options$absolute,col='black',lty=2,lwd=2)
abline(v=length(rankBy_vector)-length(superEnhancerRows),col='black ',lty=2,lwd=2)
lines(length(rankBy_vector):1,rankBy_vector[signalOrder],lwd=4, col='red')
text(0,0.8*max(rankBy_vector),paste(' Cutoff used: ',cutoff_options$absolute,'\n','Super-Enhancers identified: ',length(superEnhancerRows)),pos=4,cex=1.5)
